...
import { cfGetProblems, cfGetUserStatus, getAdaptiveProblems, getSolvedProblems, CodeforcesProblem } from '@/lib/codeforces-api'
...
export async function GET(request: NextRequest) {
  try {
    ...
    const problems: CodeforcesProblem[] = 'result' in problemsResponse ? problemsResponse.result.problems : []

    // Get user's solved problems
    const submissionsResponse = await cfGetUserStatus(handle)
    let solvedProblems = new Set<string>()
    
    if (submissionsResponse.status === 'OK' && 'result' in submissionsResponse) {
      solvedProblems = getSolvedProblems(submissionsResponse.result)
    }

    // Get adaptive problems based on user rating
    const adaptiveProblems = getAdaptiveProblems(problems, rating, solvedProblems, count)
