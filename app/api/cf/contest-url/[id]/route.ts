...
export async function GET(req: Request, context: any) {
  ...
  if (!id) {
    return NextResponse.json({ url: "https://codeforces.com/contests", status: "error" }, { status: 400 });
  }

  try {
    // Fetch the registration URL server-side (no CORS)
    // We follow redirects (default) and inspect the final response url.
    const res = await fetch(`https://codeforces.com/contestRegistration/${id}`, {
      cache: "no-store",
    });
    ...
    if (!finalUrl.includes(`/contestRegistration/${id}`)) {
      return NextResponse.json({ url: "https://codeforces.com/contests", status: "not_started" });
    }
    ...
    if (html.includes("Registration has not started") || html.includes("Before registration")) {
      return NextResponse.json({ url: "https://codeforces.com/contests", status: "not_started" });
    }

    // Otherwise, registration page is served -> open the registration link
    return NextResponse.json({ url: `https://codeforces.com/contestRegistration/${id}`, status: "open" });
  } catch (err) {
    console.error("Error checking CF contest registration:", err);
    return NextResponse.json({ url: "https://codeforces.com/contests", status: "error" });
  }
}
