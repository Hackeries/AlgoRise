...
export async function GET(request: NextRequest) {
  try {
    ...
    if (contestsResponse.status !== 'OK' || !('result' in contestsResponse) || !contestsResponse.result) {
    ...
    console.log(`Fetched ${allContests.length} contests from Codeforces`)
    console.log(`Found ${upcomingContests.length} upcoming contests, ${recentContests.length} recent contests`)

    return NextResponse.json({
      upcoming: upcomingContests.map(contest => ({
        ...contest,
        timeUntilStart: contest.startTimeSeconds ? contest.startTimeSeconds - now : 0,
        registrationOpen: contest.startTimeSeconds ? (contest.startTimeSeconds - now) > 0 : false
