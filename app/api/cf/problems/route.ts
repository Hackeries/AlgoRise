...
import { cfGetProblems, cfGetUserStatus, getSolvedProblems, CodeforcesProblem } from '@/lib/codeforces-api'
...
export async function GET(request: NextRequest) {
  try {
    ...
    console.log(`Fetching problems for rating ${rating}, tags: [${tags.join(', ')}], count: ${count}`)
    ...
    const grouped = {
      easier: filteredProblems.filter((p: CodeforcesProblem) => p.rating && p.rating < rating - 50),
      target: filteredProblems.filter((p: CodeforcesProblem) => p.rating && Math.abs(p.rating - rating) <= 50),
      harder: filteredProblems.filter((p: CodeforcesProblem) => p.rating && p.rating > rating + 50),
...
function calculateScore(problem: any, targetRating: number, preferredTags: string[]): number {
  let score = 100
  ...
  const matchingTags = problem.tags.filter((tag: string) => preferredTags.includes(tag))
