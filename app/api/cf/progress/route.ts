...
import { cfGetUserStatus, cfGetUserRating, calculateUserProgress } from '@/lib/codeforces-api'
...
export async function GET(request: NextRequest) {
  try {
    ...
    const progress = 'result' in submissionsResponse ? calculateUserProgress(submissionsResponse.result) : null
    
    // Calculate streak data
    const today = new Date()
    const streakData = 'result' in submissionsResponse ? calculateStreak(submissionsResponse.result) : null
    
    // Calculate weekly/monthly trends
    const trends = 'result' in submissionsResponse ? calculateTrends(submissionsResponse.result) : null
...
function calculateTrends(submissions: any[]): {
  ...
  const thisWeekSolved = submissions.filter(sub => 
    sub.verdict === 'OK' && sub.creationTimeSeconds > now - oneWeek
  ).length
  
  const lastWeekSolved = submissions.filter(sub => 
    sub.verdict === 'OK' && 
    sub.creationTimeSeconds > now - 2 * oneWeek && 
    sub.creationTimeSeconds <= now - oneWeek
  ).length
  
  const thisMonthSolved = submissions.filter(sub => 
    sub.verdict === 'OK' && sub.creationTimeSeconds > now - oneMonth
  ).length
  
  const lastMonthSolved = submissions.filter(sub => 
    sub.verdict === 'OK' && 
  ...
  return {
    ...
    weeklyTrend: thisWeekSolved > lastWeekSolved ? 'up' : 
                 thisWeekSolved < lastWeekSolved ? 'down' : 'stable',
    monthlyTrend: thisMonthSolved > lastMonthSolved ? 'up' : 
                  thisMonthSolved < lastMonthSolved ? 'down' : 'stable'
  }
}
