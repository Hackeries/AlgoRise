...
export async function POST() {
  try {
    ...
    // Fetch CF handle row (no generic)
    const { data, error: selErr } = await supabase
      .from('cf_handles')
      .select('handle, verification_token, verified, user_id')
      .eq('user_id', user.id)
      .single();

    if (selErr || !data) {
      return NextResponse.json({ error: 'no handle to verify' }, { status: 400 });
    }

    const row = data as CFHandleRow;

    if (row.verified) {
      return NextResponse.json({ verified: true, rating: null });
    }

    const cf = await fetchCfUser(row.handle);
    const org = (cf.organization || '').toLowerCase();
    ...
    if (!matched) {
      return NextResponse.json({
        verified: false,
        reason: 'token not found in organization',
        rating: null,
      });
    }

    // Update verification status (no generic)
    ...
    // Insert snapshot (no generic)
    const snapshot: CFSnapshotRow = {
      user_id: user.id,
      handle: cf.handle,
    ...
    const { error: snapErr } = await supabase.from('cf_snapshots').insert(snapshot);
