...
export async function POST(_: Request, { params }: { params: { id: string } }) {
  ...
  if (contest.host_user_id !== user.id) return NextResponse.json({ error: "forbidden" }, { status: 403 })
  ...
  const withRank = list.map((r, idx) => ({ user_id: r.user_id, score: r.solved, penalty_s: r.penalty, rank: idx + 1 }))

  // fetch pre-contest ratings snapshot (fallback 1200)
  // for v1, we reuse cf_snapshots.last_rating if present, else 1200
  const { data: snaps } = await supabase.from("cf_snapshots").select("user_id, last_rating")
  ...
  const { error: updErr } = await supabase.from("contests").update({ status: "ended" }).eq("id", contestId)
