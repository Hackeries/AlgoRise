...
export async function GET(
  ...
  const { data: rawContest, error } = await supabase
    .from('contests')
    .select('*')
    .eq('id', contestId)
    .single();
  ...
  const now = Date.now();
  const startsAt = rawContest.starts_at
    ? new Date(rawContest.starts_at).getTime()
    : null;
  const endsAt = rawContest.ends_at
    ? new Date(rawContest.ends_at).getTime()
    : null;
