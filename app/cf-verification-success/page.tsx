...
export default function VerificationSuccessPage() {
  ...
  const fetchUserStats = async (handle: string, basicStats: Partial<UserStats>) => {
    try {
      ...
      const contestsParticipated = ratingData.status === 'OK' ? ratingData.result.length : 0
  ...
  const generateAdaptiveSheet = () => {
    ...
    router.push(`/adaptive-sheet?handle=${userStats.handle}&minRating=${minRating}&maxRating=${maxRating}`)
  }
      ...
      <Card className="w-full max-w-2xl bg-slate-800/50 border-slate-700">
        ...
        <CardContent className="space-y-6">
          ...
          <div className="text-center">
            <h3 className="text-lg font-semibold text-white mb-2">Welcome, {userStats.handle}!</h3>
          ...
          <div className="bg-gradient-to-r from-blue-500/10 to-purple-500/10 p-6 rounded-lg border border-blue-500/20">
            ...
            <p className="text-slate-300 mb-4">
              Get personalized problem recommendations based on your current rating ({userStats.rating}).
              We'll suggest problems from {Math.max(800, userStats.rating - 100)} to {userStats.rating + 200} difficulty.
            </p>
            <Button 
