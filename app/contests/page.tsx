...
export default function ContestsPage() {
  ...
  const createContest = async () => {
    ...
    if (selectedStart.getTime() < oneHourFromNow.getTime()) {
      ...
      toast({
        ...
        description: "Contest start time must be at least 1 hour from now. Time has been adjusted automatically.",
        variant: "default",
      })
      return
    }
    ...
    const endDateTime = new Date(selectedStart.getTime() + durationMinutes * 60 * 1000).toISOString()
    ...
    try {
      ...
      if (response.ok) {
        ...
        setCreatedContestLink(`${baseUrl}/contests/${data.contest.id}/participate`)
        ...
        toast({
          title: "Error",
          description: data?.error || data?.details || "Failed to create contest",
          variant: "destructive",
        })
      }
    } catch (error: any) {
  ...
  const handleJoinPrivateContest = async (contest: PrivateContest) => {
    ...
    if (contest.status === "ended" || (contest.ends_at && now >= end)) {
      toast({
        title: "Contest Ended",
        description: "This contest has already ended. You can view details or leaderboard instead.",
        variant: "destructive",
      })
      return
    }
      ...
      privateContests.forEach((c) => {
        ...
        if (isLive && !alreadyNotified) {
          ...
          toast({
            ...
            action: (
              <Button
                onClick={() => window.open(`/contests/${c.id}/participate`, "_blank")}
            ...
            <div className="overflow-y-auto max-h-[60vh] space-y-6 py-4 px-2 sm:px-4">
              ...
              <div className="space-y-2">
                ...
                <Select
                  ...
                  onValueChange={(value) => setFormData((prev) => ({ ...prev, contestMode: value }))}
                >
                  <SelectTrigger>
                    <SelectValue placeholder="Select contest mode" />
                  </SelectTrigger>
                  <SelectContent>
                    <SelectItem value="practice">Practice Arena (5-12 problems, 2-4 hours)</SelectItem>
                    <SelectItem value="icpc">ICPC Arena (10-13 problems, 5 hours)</SelectItem>
                  </SelectContent>
                </Select>
              ...
              <div className="space-y-4">
                ...
                <p className="text-xs text-muted-foreground">
                  Note: Contest must start at least 1 hour from now. Time will be adjusted automatically if needed.
                </p>
                ...
                {formData.problemCount && (
                  <div className="text-sm text-muted-foreground">
                    ...
                    {formData.contestMode === "icpc"
                      ...
                      : `${getAutoDuration(Number.parseInt(formData.problemCount)).hours} hours (auto-calculated)`}
                  </div>
                )}
              </div>
                ...
                <div className="space-y-2">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    >
                      ...
                      <SelectContent>
                        {Array.from({ length: 28 }, (_, i) => 800 + i * 100).map((val) => (
                  ...
                  <p className="text-xs text-muted-foreground">
                    Problems will be randomly selected from Codeforces (rating 800-3500).
                  </p>
                </div>
              </div>
              ...
              <div className="space-y-4">
                ...
                <div className="flex items-center justify-between">
                  <div className="space-y-0.5">
                    ...
                    <p className="text-sm text-muted-foreground">Participants can join after contest starts</p>
                  </div>
                  <Switch
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="text-sm text-muted-foreground space-y-1">
                      ...
                      <p>
                        ...
                        {formData.contestMode === "icpc"
                          ? "5 hours"
                          : `${getAutoDuration(Number.parseInt(formData.problemCount)).hours} hours`}
                      </p>
                      <p>
                        <strong>Problems:</strong> {formData.problemCount} (Rating {formData.ratingMin}-
                        {formData.ratingMax})
                      </p>
                    </div>
                  </div>
              ...
              <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
                {upcomingCfContests.slice(0, 6).map((contest) => (
                  <Card
                    ...
                    onClick={() =>
                      handleCodeforcesContestClick(contest.id, contest.startTimeSeconds || 0, contest.name)
                    }
                  >
                  ...
                  <Card key={contest.id} className="hover:bg-white/5 transition-colors">
                    <CardHeader className="pb-3">
                      ...
                      <CardDescription className="text-xs">
                        {contest.visibility === "public" ? "Public" : "Private"} • Created by{" "}
                        {contest.isHost ? "You" : "Others"}
                      </CardDescription>
                    </CardHeader>
                    <CardContent className="pt-0">
      ...
      <Dialog open={shareDialogOpen} onOpenChange={setShareDialogOpen}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>
              Share this contest link with your friends. Users can register up to 10 minutes before start.
            </DialogDescription>
          </DialogHeader>
          <div className="flex items-center gap-2 mt-4">
            <Input value={createdContestLink || ""} readOnly className="flex-1" />
            <Button
              onClick={() => {
                if (createdContestLink) navigator.clipboard.writeText(createdContestLink)
                toast({
                  title: "Copied!",
