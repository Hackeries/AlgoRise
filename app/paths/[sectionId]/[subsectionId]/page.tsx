...
export default function SubsectionPage() {
  ...
  const loadUserProgress = async () => {
    try {
      ...
      const solved = new Set(data?.filter((item) => item.solved).map((item) => item.problem_id) || [])
      const revision = new Set(data?.filter((item) => item.marked_for_revision).map((item) => item.problem_id) || [])
  ...
  const stars = Math.min(5, Math.floor(solvedCount / Math.max(1, Math.ceil(totalProblems / 5))))
  ...
  const ordered = recommendedOrder
    ? [...filtered].sort((a, b) => (difficultyRank[a.difficulty] ?? 99) - (difficultyRank[b.difficulty] ?? 99))
    : filtered
    ...
    <main className="mx-auto max-w-4xl px-4 py-10">
      ...
      <Card className="mb-8">
        <CardHeader>
          <div className="flex items-center gap-3 mb-4">
            <div className="p-3 rounded-lg bg-blue-500/20 text-2xl">{section.icon}</div>
            <div>
            ...
            {!loading && (
              <>
                <div className="flex items-center gap-2">
                  ...
                  <span>
                    {solvedCount}/{totalProblems} completed ({completionPercentage}%)
                  </span>
                </div>
                <div className="flex items-center gap-2">
          ...
          <div className="flex items-center justify-between mb-2">
            ...
            <div className="text-sm">
              ...
              {Array.from({ length: 5 }).map((_, i) => (
                <span key={i} className={i < stars ? "text-yellow-500" : "text-muted-foreground"}>
                  ★
                </span>
              ))}
            </div>
      ...
      <Card className="mb-6">
        <CardHeader>
          ...
          <CardDescription>Search, filter and order problems to fit your pace.</CardDescription>
        </CardHeader>
        <CardContent>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Complete these problems to master {subsection.title}. Click the circle to mark as solved and the star to
            mark for revision.
          </CardDescription>
        </CardHeader>
        <CardContent>
            ...
            <div className="space-y-3">
              ...
              {ordered.map((problem, index) => {
                ...
                return (
                  ...
                  >
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <div className="text-xs text-muted-foreground">
                          ...
                          <span className="mr-2">Source: {problem.platform}</span>
                          {problem.tags?.length ? (
                            <span>
                    ...
                    <div className="flex items-center gap-2">
                      <Badge variant={isSolved ? "default" : "outline"}>{isSolved ? "Solved" : "Pending"}</Badge>
              ...
              {ordered.length === 0 && (
                <div className="py-8 text-center text-muted-foreground text-sm">
                  No problems match your filters. Try clearing the search or filters.
                </div>
              )}
            </div>
          )}
