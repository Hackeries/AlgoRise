...
import { CheckCircle2, Mail, Bell, User, Shield, SettingsIcon } from "lucide-react"
  ...
  <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 sm:gap-4 p-3 sm:p-4 rounded-lg border bg-muted/30 transition-all hover:bg-muted/50 hover:border-primary/20">
    <div className="space-y-1.5 flex-1">
      <div className="flex items-center gap-2">
        ...
        <Label className="text-sm sm:text-base font-medium cursor-pointer">{label}</Label>
      </div>
      <p className="text-xs sm:text-sm text-muted-foreground leading-relaxed">{description}</p>
      <div className="flex items-center gap-2 pt-1">
        <div className={`h-2 w-2 rounded-full transition-colors ${enabled ? "bg-green-500" : "bg-red-500"}`} />
        <span
    ...
    <Switch checked={enabled} onCheckedChange={onToggle} className="transition-all" />
  </div>
)
  ...
  const handleToggle = useCallback(
    (key: keyof typeof preferences) => {
      setPreferences((prev) => {
        ...
        toast({
          title: `${
            key === "emailNotifications" ? "Email Notifications" : "Daily Reminders"
            ...
            <AnimatedSection delay={0.26}>
              <Card className="border-2 transition-all duration-200 hover:shadow-lg hover:border-primary/20">
                <CardHeader className="space-y-1 pb-4">
                  ...
                  <CardDescription className="text-sm sm:text-base">Customize your AlgoRise experience</CardDescription>
                </CardHeader>
