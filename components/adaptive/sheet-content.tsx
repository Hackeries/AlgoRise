...
export function AdaptiveSheetContent({
  ...
  function handleCompleted(item: SheetItem) {
    postAction(
      ...
      toast({ title: 'Marked as completed — great job!', description: `Next review: ${fmtDue(item.nextDueAt)}` });
      try {
        ...
        if (payload?.newLongest === true && typeof payload?.currentStreak === 'number') {
          toast({ title: '🏆 New longest streak!', description: `${payload.currentStreak} days — keep the momentum!` });
        }
      } catch {}
    });
  }
  ...
  function saveNotes() {
    if (currentProblem) {
      ...
      toast({ title: 'Notes saved', description: 'Your learning notes have been saved locally.' });
    }
  ...
  function handleSkip(item: SheetItem) {
    postAction('/api/adaptive-sheet/skip', { problemId: item.id }, item.id).then(() =>
      toast({ title: 'Skipped', description: 'We’ll resurface this later.' })
    );
  }

  function handleFail(item: SheetItem) {
    postAction('/api/adaptive-sheet/fail', { problemId: item.id }, item.id).then(() =>
      toast({ title: 'Marked as failed', description: 'Recovery mode will adapt your next set.' })
    );
  }
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.5 }} className='space-y-6'>
      <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }}>
        <AdaptiveFilterBar initialRatingBase={filters.ratingBase} initialTags={filters.tags} onChange={s => setFilters(s)} />
      </motion.div>
      ...
      <AnimatePresence mode='wait'>
        {isLoading ? (
          ...
          <motion.div key='error' initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className='flex items-center justify-center py-12'>
          ...
          <motion.div key='empty' initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className='flex items-center justify-center py-16'>
            <div className='text-center space-y-4 max-w-md'>
              <div className='text-6xl'>🎯</div>
              <h3 className='text-lg font-semibold text-muted-foreground'>No problems found</h3>
              <p className='text-sm text-muted-foreground'>Try adjusting your filters or rating range to find more problems.</p>
            </div>
          </motion.div>
        ) : (
          <motion.div key='content' initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className='space-y-8'>
            {sections.map((section, sectionIndex) =>
              section.items.length ? (
                <motion.section key={section.key} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4, delay: sectionIndex * 0.1 }} className={`space-y-4 p-4 sm:p-6 rounded-xl border-2 ${getSectionColor(section.key)}`}>
                  <motion.div initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.3, delay: sectionIndex * 0.1 + 0.2 }} className='flex items-center gap-3'>
                  ...
                  <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.5, delay: sectionIndex * 0.1 + 0.3 }} className='grid gap-4 sm:gap-6 grid-cols-1 lg:grid-cols-2 xl:grid-cols-3'>
                    <AnimatePresence>
                      {section.items.map((item, itemIndex) => (
                        <motion.div key={item.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20, scale: 0.95 }} transition={{ duration: 0.3, delay: sectionIndex * 0.1 + itemIndex * 0.05 + 0.4 }} layout>
                          <AdaptiveProblemCard
                            problem={{ id: item.id, title: item.problem.title, url: item.problem.url, rating: item.problem.rating, tags: item.problem.tags }}
      ...
      <Dialog open={notesDialogOpen} onOpenChange={setNotesDialogOpen}>
        <DialogContent className='max-w-2xl'>
          <DialogHeader>
            <DialogTitle className='text-xl'>📝 Notes for: {currentProblem?.problem.title}</DialogTitle>
          </DialogHeader>
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3 }} className='space-y-6'>
            <div className='flex flex-wrap gap-2'>
              <Badge variant='outline' className='font-medium'>⭐ Rating {currentProblem?.problem.rating}</Badge>
            ...
            <div className='space-y-2'>
              <label className='text-sm font-medium text-muted-foreground'>Your learning notes</label>
              <Textarea
