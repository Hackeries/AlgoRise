...
export function CFVerificationDebug() {
  ...
  const testDatabase = async () => {
    ...
    try {
      ...
      if (snapshotError) {
        setDbTest(
          `Snapshot Error: ${snapshotError.code} - ${snapshotError.message}`
        );
        return;
      }

      setDbTest(
        `✅ Database OK - Handle: ${handleData?.length || 0} records, Snapshots: ${snapshotData?.length || 0} records`
      );
    } catch (error) {
