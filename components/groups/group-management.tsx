...
import { UserPlus, Users, Mail, Crown, Shield, User, MoreVertical, Copy, Check } from 'lucide-react'
...
export function GroupManagement({ groupId, groupName, userRole }: GroupManagementProps) {
        ...
        <CardContent>
          {isLoading ? (
            ...
            <div className="space-y-2">
              {members.map((member) => (
                <div key={member.id} className="flex items-center space-x-4 p-3 rounded-lg hover:bg-muted/20">
      ...
      {invites.length > 0 && (
        <Card>
          ...
          <CardContent>
            <div className="space-y-2">
              {invites.map((invite) => (
                <div key={invite.id} className="flex items-center justify-between p-3 rounded-lg bg-muted/20">
                  <div>
                    <p className="text-sm font-medium">{invite.email}</p>
                    <p className="text-xs text-muted-foreground">
                      Invited {new Date(invite.createdAt).toLocaleDateString()} • Role: {invite.role}
                    </p>
                  </div>
