...
export default function ModernLanding() {
  ...
  const fetchUserStats = async (handleOverride?: string) => {
    ...
    try {
      const response = await fetch(`/api/cf/profile?handle=${encodeURIComponent(handle)}`, { cache: "no-store" })
      ...
      if (error instanceof Error) {
        if (error.message.includes("404")) msg = `User '${handle}' not found on Codeforces.`
