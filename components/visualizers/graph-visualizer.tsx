...
export function GraphVisualizer() {
  ...
  const initializeGraph = () => {
    const sampleNodes: Node[] = [
      { id: 0, x: 100, y: 100, visited: false, distance: Infinity, parent: undefined, inQueue: false, isStart: true, isEnd: false },
      { id: 1, x: 250, y: 50, visited: false, distance: Infinity, parent: undefined, inQueue: false, isStart: false, isEnd: false },
      { id: 2, x: 400, y: 100, visited: false, distance: Infinity, parent: undefined, inQueue: false, isStart: false, isEnd: false },
      { id: 3, x: 150, y: 200, visited: false, distance: Infinity, parent: undefined, inQueue: false, isStart: false, isEnd: false },
      { id: 4, x: 350, y: 200, visited: false, distance: Infinity, parent: undefined, inQueue: false, isStart: false, isEnd: true },
      { id: 5, x: 250, y: 280, visited: false, distance: Infinity, parent: undefined, inQueue: false, isStart: false, isEnd: false },
    ]
  ...
  const bfs = (): AnimationStep[] => {
    ...
    const nodesCopy: Node[] = nodes.map(n => ({ ...n, visited: false, inQueue: false, parent: undefined }))
    ...
    while (queue.length > 0) {
      ...
      const neighbors = edges
        ...
        .filter(neighbor => !nodesCopy[neighbor].visited && !nodesCopy[neighbor].inQueue)
  ...
  const dfs = (): AnimationStep[] => {
    const steps: AnimationStep[] = []
    const nodesCopy: Node[] = nodes.map(n => ({ ...n, visited: false, parent: undefined }))
  ...
  const dijkstra = (): AnimationStep[] => {
    ...
    while (true) {
      ...
      const neighborEdges = edges.filter(e => e.from === currentNode.id || e.to === currentNode.id)
      ...
      <CardContent className="space-y-6">
        ...
        <div className="flex flex-wrap gap-4 items-center">
          <div className="flex items-center gap-2">
            ...
            <Select value={algorithm} onValueChange={(value: GraphAlgorithm) => setAlgorithm(value)} disabled={isPlaying}>
          ...
          <div className="flex items-center gap-2">
            ...
            <Select value={startNode.toString()} onValueChange={(value) => setStartNode(Number(value))} disabled={isPlaying}>
              <SelectTrigger className="w-16">
                <SelectValue />
              </SelectTrigger>
              <SelectContent>
                {nodes.map(node => (
                  <SelectItem key={node.id} value={node.id.toString()}>{node.id}</SelectItem>
                ))}
              </SelectContent>
            </Select>
          </div>
          ...
          <div className="flex items-center gap-2">
            ...
            <Select value={endNode.toString()} onValueChange={(value) => setEndNode(Number(value))} disabled={isPlaying}>
              <SelectTrigger className="w-16">
                <SelectValue />
              </SelectTrigger>
              <SelectContent>
        ...
        <div className="flex gap-2">
          {!isPlaying ? (
            <Button onClick={startAlgorithm} className="flex items-center gap-2">
            ...
            <CardContent className="text-sm space-y-2">
              {algorithm === 'bfs' && (
                <>
                  ...
                  <p><strong>Use Case:</strong> Shortest path in unweighted graphs</p>
                </>
              )}
              {algorithm === 'dfs' && (
                <>
                  ...
                  <p><strong>Use Case:</strong> Cycle detection, topological sorting</p>
                </>
              )}
              {algorithm === 'dijkstra' && (
                <>
                  <p><strong>Time Complexity:</strong> O(V² ) or O(E + V log V)</p>
                  <p><strong>Space Complexity:</strong> O(V)</p>
                  <p><strong>Data Structure:</strong> Priority Queue</p>
          ...
          <Card>
            <CardHeader>
              <CardTitle className="text-lg">
                {algorithm === 'bfs' ? 'Queue Status' : 
                 algorithm === 'dfs' ? 'Stack Status' : 'Statistics'}
              </CardTitle>
            </CardHeader>
            <CardContent className="text-sm space-y-2">
              {(algorithm === 'bfs' || algorithm === 'dfs') && currentStepData && (
                <>
                  <p><strong>Contents:</strong> [{(currentStepData.queue || currentStepData.stack || []).join(', ')}]</p>
                  <p><strong>Size:</strong> {(currentStepData.queue || currentStepData.stack || []).length}</p>
                </>
              )}
              ...
              <p><strong>Target Node:</strong> {endNode}</p>
            </CardContent>
          </Card>
        </div>
      </CardContent>
