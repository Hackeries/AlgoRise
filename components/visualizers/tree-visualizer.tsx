...
export function TreeVisualizer() {
  ...
  const resetVisualization = () => {
    setTree(prev => prev.map(node => ({ ...node, visited: false, current: false })))
    setCurrentStep(0)
    setSteps([])
    setVisitedOrder([])
    setIsPlaying(false)
  ...
  const inorderTraversal = (root: TreeNode, steps: AnimationStep[], visitedOrder: number[]): number[] => {
    ...
    // Visit left subtree
    if (root.left) {
      currentVisitedOrder = inorderTraversal(root.left, steps, currentVisitedOrder)
    }
    ...
    // Visit right subtree
    if (root.right) {
      currentVisitedOrder = inorderTraversal(root.right, steps, currentVisitedOrder)
    }
  ...
  const preorderTraversal = (root: TreeNode, steps: AnimationStep[], visitedOrder: number[]): number[] => {
    if (!root) return visitedOrder
    ...
    // Visit left subtree
    if (root.left) {
      currentVisitedOrder = preorderTraversal(root.left, steps, currentVisitedOrder)
    }

    // Visit right subtree
    if (root.right) {
      currentVisitedOrder = preorderTraversal(root.right, steps, currentVisitedOrder)
    }
  ...
  const postorderTraversal = (root: TreeNode, steps: AnimationStep[], visitedOrder: number[]): number[] => {
    if (!root) return visitedOrder
    ...
    // Visit left subtree
    if (root.left) {
      currentVisitedOrder = postorderTraversal(root.left, steps, currentVisitedOrder)
    }

    // Visit right subtree
    if (root.right) {
      currentVisitedOrder = postorderTraversal(root.right, steps, currentVisitedOrder)
    }
          ...
          <div className="bg-muted/10 rounded-lg p-6 min-h-[400px]">
            <svg width="100%" height="400" className="overflow-visible">
              ...
              {currentStepData.nodes.map((node) => {
                const edges = []
                if (node.left) {
                  const leftNode = currentStepData.nodes.find(n => n.id === node.left!.id)
                  if (leftNode) {
                    edges.push(
                      <line
                ...
                if (node.right) {
                  const rightNode = currentStepData.nodes.find(n => n.id === node.right!.id)
                  if (rightNode) {
                    edges.push(
                      <line
            ...
            <div className="mt-4 p-3 bg-muted rounded-lg">
              ...
              {currentStepData.visitedOrder.length > 0 && (
                <p className="text-xs text-muted-foreground mt-1">
                  Visited order: {currentStepData.visitedOrder.map(id => 
                    currentStepData.nodes.find(n => n.id === id)?.value
                  ).join(' → ')}
                </p>
              )}
            </div>
