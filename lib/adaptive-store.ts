import type { AdaptiveSheetResponse, Problem, SheetItem, WeakTagStats } from "@/lib/types"
import { Platform, Outcome } from "@/lib/types"
...
export function getAdaptiveSheet(userId: string, baseRating = 1700, tags: string[] = []): AdaptiveSheetResponse {
  ...
  const candidates = PROBLEMS.filter((p) => {
    ...
    return r >= minR && r <= maxR && (tags.length === 0 || tags.some((t) => p.tags.includes(t)))
  })
...
export function updateOutcome(userId: string, problemId: string, outcome: Outcome): AdaptiveSheetResponse {
  ...
  state.lastInteractionAt = new Date(nowTs())
