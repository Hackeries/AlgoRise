...
--  - Correctly drops existing policies without referencing a missing p.tablename
--    by using constant table names in EXECUTE format() calls.
--  - Retains all previous improvements: updated_at triggers, guardrails, status
--    transitions, service_role bypass, helper functions, and indexes.
-- =========================================================================
...
declare p record;
begin
  -- Drop policies on group_challenges
  for p in
  ...
  -- Drop policies on group_challenge_progress
  for p in
    select policyname from pg_policies
