...
export function AdaptiveRightRailData({
  ...
  const totalCandidates =
    (data?.groups.dueNow?.length || 0) +
    (data?.groups.dueSoon?.length || 0) +
    (data?.groups.later?.length || 0);
  const solved = Math.round(
    (data?.stats.solvedRate || 0) * Math.max(1, totalCandidates)
  );

  // Use real CF data for solved problems and streak
  const realSolved = cfProgress?.progress?.totalSolved || solved;
  const realStreak =
    cfProgress?.progress?.streakData?.current || data?.stats.streak || 0;
  ...
  // Fallback 2: Use adaptive sheet data
    ...
    weakTagItems = Object.entries(data.stats.weakTags).map(([tag, v]) => {
      const acc =
        v.attempts > 0
          ? Math.round(((v.attempts - v.fails) / v.attempts) * 100)
          : 0;
      return { tag, acc };
    });
  }
  ...
  if (
    ratingChange === 0 &&
    cfProgress?.progress?.ratingHistory &&
    cfProgress.progress.ratingHistory.length >= 2
  ) {
    ratingChange =
      cfProgress.progress.ratingHistory[
        cfProgress.progress.ratingHistory.length - 1
      ].change;
  }
              ...
              >
                ...
                >
                  <motion.div
                    initial={{ width: 0 }}
                    animate={{
                      width: `${Math.max(5, Math.min(item.acc, 100))}%`,
                    }}
        ...
        <CardContent className='space-y-3'>
          ...
          >
            ...
            <p className='text-red-600'>
              ...
              <span className='font-semibold'>-300 to -150</span> rating with
              your favorite tags to rebuild confidence.
            </p>
            <p className='text-xs text-red-500 mt-2'>
              This mode activates automatically after significant rating drops
              or contest struggles.
            </p>
          </motion.div>
        </CardContent>
      </Card>
