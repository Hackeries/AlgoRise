...
function toSheetItem(r: DbItem) {
  // clamp ease into 1|2|3|4|5
  const eRaw = Number(r.ease ?? 3)
  const eClamped = Math.min(5, Math.max(1, Math.round(eRaw))) as 1 | 2 | 3 | 4 | 5

  return {
    id: r.problem_id,
    problem: {
      id: r.problem_id,
      platform: Platform.Codeforces, // all adaptive items are CF-backed
    ...
    nextDueAt: r.next_due_at ? new Date(r.next_due_at) : new Date(), // convert to Date
...
export async function fetchAdaptiveSheetDb(
  ...
  const selectCols =
    "problem_id,rating,tags,repetitions,ease,interval_days,next_due_at,last_outcome,problem_title,problem_url"
