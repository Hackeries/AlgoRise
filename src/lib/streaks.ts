...
export async function applyMeaningfulAction(
  ...
  if (existing) {
    const last = existing.last_active_day
      ? new Date(existing.last_active_day)
      : undefined;
  ...
  return (
    (data as unknown as StreakRow) ?? (upsertPayload as unknown as StreakRow)
  );
}
...
export async function updateStreakForUser(userId: string) {
  ...
  const row = await applyMeaningfulAction(
